...
model = os.getenv("OPENROUTER_MODEL", "nousresearch/deephermes-3-llama-3-8b-preview:free")
...
try:
    ...
    print(f"Sending request to OpenRouter API at {start_time.strftime('%H:%M:%S')}...")
    ...
    if 'invalid_api_key' in str(e).lower():
        print(f"\nThe API key provided seems to be invalid. Please check your OPENROUTER_API_KEY.")
    elif 'rate limit' in str(e).lower():
        print(f"\nYou've hit a rate limit. Please try again later.")
    else:
        print(f"\nAn error occurred while calling the OpenRouter API.")
